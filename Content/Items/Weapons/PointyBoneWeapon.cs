...
using TheSkeletronMod.Content.Items.Weapons.Range;

namespace TheSkeletronMod.Content.Items.Weapons
{
    internal class PointyBoneWeapon : ModItem
    {
        public override string Texture => SkeletronUtils.GetEntityTexture<AncientBoneDart>();
                ...
                if (percentageChargeUp == 1 && Projectile.ai[0] == 0)
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        for (int l = 0; l < 50; l++)
                        {
                            ...
                            float randomrotate = MathHelper.Lerp(50f, 1f, Easing.InOutSine(multiplier));
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (!isReleased)
            {
                ...
                for (int l = 0; l < MathHelper.Lerp(0, 5, percentageChargeUp); l++)
                {
                    for (int i = 0; i < 4; i++)
                    {
                        Vector2 finaldrawPos = drawpos.IgnoreTilePositionOFFSET(Vector2.UnitY.RotatedBy(MathHelper.ToRadians(MathHelper.Lerp(0 + 90f * i, 270f + 90f * i, Easing.InOutSine(percentageChargeUp)))), MathHelper.Lerp(180, 2, Easing.InOutSine(percentageChargeUp)));
