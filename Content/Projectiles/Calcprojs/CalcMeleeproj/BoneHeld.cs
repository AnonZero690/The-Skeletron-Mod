...
namespace TheSkeletronMod.Content.Projectiles.Calcprojs.CalcMeleeproj
{
    public class BoneHeld : ModProjectile
    {
        ...
        public void Charge(Player player)
        {
            ...
            if (player.GetModPlayer<SkeletronPlayer>().cc < 10)
            {
                velocity = 3f;
            }
            if (player.GetModPlayer<SkeletronPlayer>().cc > 10 && player.GetModPlayer<SkeletronPlayer>().cc < 20)
            {
                velocity = 4f;
            }
            if (player.GetModPlayer<SkeletronPlayer>().cc > 20 && player.GetModPlayer<SkeletronPlayer>().cc < 30)
            {
                velocity = 5f;

            }
            if (player.GetModPlayer<SkeletronPlayer>().cc > 30 && player.GetModPlayer<SkeletronPlayer>().cc < 40)
            {
                velocity = 6f;
                Projectile.damage = dmg * 2;
            }
            if (player.GetModPlayer<SkeletronPlayer>().cc > 40 && player.GetModPlayer<SkeletronPlayer>().cc < 50)
            {
                velocity = 7f;
            }
            if (player.GetModPlayer<SkeletronPlayer>().cc > 50 && player.GetModPlayer<SkeletronPlayer>().cc < 60)
            {
                velocity = 8f;

            }
            if (player.GetModPlayer<SkeletronPlayer>().cc > 60 && player.GetModPlayer<SkeletronPlayer>().cc < 70)
            {
                velocity = 9f;
                Projectile.damage = dmg * 3;
            }
            if (player.GetModPlayer<SkeletronPlayer>().cc > 70 && player.GetModPlayer<SkeletronPlayer>().cc < 80)
            {
                velocity = 10f;

            }
            if (player.GetModPlayer<SkeletronPlayer>().cc > 80 && player.GetModPlayer<SkeletronPlayer>().cc < 90)
            {
                velocity = 11f;
            }
            if (player.GetModPlayer<SkeletronPlayer>().cc > 90 && player.GetModPlayer<SkeletronPlayer>().cc < 100)
            {
                velocity = 12f;
            }
            if (player.GetModPlayer<SkeletronPlayer>().cc >= 100)
            {
                velocity = 13f;
        ...
        public override void AI()
        {
            ...
            if (player.GetModPlayer<SkeletronPlayer>().cc == 30)
            {
                for (int i = 0; i < 100; i++)
                {
                    Vector2 speed = Main.rand.NextVector2CircularEdge(1f, 1f);
            ...
            if (player.GetModPlayer<SkeletronPlayer>().cc == 60)
            {
                for (int i = 0; i < 100; i++)
                {
                    Vector2 speed = Main.rand.NextVector2CircularEdge(1f, 1f);
            ...
            if (player.GetModPlayer<SkeletronPlayer>().cc == 99)
            {
                for (int i = 0; i < 100; i++)
                {
                    Vector2 speed = Main.rand.NextVector2CircularEdge(1f, 1f);
            ...
            if (player.GetModPlayer<SkeletronPlayer>().cc < 100)
            {
                player.GetModPlayer<SkeletronPlayer>().cc++;
            }
            else if (maximunCharge > 100)
            {
                player.GetModPlayer<Screenshake>().ScreenShake = 1;
            }
            player.GetModPlayer<SkeletronPlayer>().ShowChargeBar = true;
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            player.GetModPlayer<SkeletronPlayer>().ShowChargeBar = false;
            player.GetModPlayer<SkeletronPlayer>().cc = 0;
        }
