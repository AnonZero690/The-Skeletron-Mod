    ...
    public class BoneScytheP : ModProjectile
    {
        ...
        public override void Kill(int timeleft)
        {
            ...
            for (int a = 0; a < 80; a++)
            {
                ...
                var d = Dust.NewDustPerfect(Projectile.Center, DustID.WhiteTorch, speed * 5, Scale: 3f);
                ;
                d.noGravity = true;
            }
            for (int b = 0; b < 80; b++)
            {
                Vector2 speed = Main.rand.NextVector2CircularEdge(3f, 3f);
                var d = Dust.NewDustPerfect(Projectile.Center, DustID.PurpleTorch, speed * 5, Scale: 3f);
                ;
                d.noGravity = true;
            }
            for (int i = 0; i < 80; i++)
